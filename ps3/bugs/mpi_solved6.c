...
/************** BUG: nreqs is not accurate *****************/
/*
 * Solution: Set offset to 0. Every process gets their own copy of MPI_Request and MPI_Status,
 * so there's no need for offset.
 */
...
/* Task 2 does the blocking send operation */
  if (rank == 2) {
		/* Solution: nreqs = 0 since task 2 makes no async calls */
		nreqs = 0;
    dest = 3;
    for (i=0; i<REPS; i++) {
...
/* Task 3 does the non-blocking receive operation */
  if (rank == 3) {
    ...
    src = 2;
    offset = 0;
		/* Solution: nreqs = REPS since task 3 makes REPS async calls */
    for (i=0; i<REPS; i++) {
