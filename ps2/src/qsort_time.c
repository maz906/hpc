...
void basic_qsort_time(int size, int increment, int times) 
{
	...
	double int_time_std = 0, double_time_std = 0, float_time_std = 0, long_time_std = 0, point_time_std = 0, vec_time_std = 0;
	for (j = increment; j <= size; j += increment)
	{
		for (i = 0; i < times; ++i)
		{
			...
			vec_time_single = time_qsort_array(rand_vec, j, sizeof(Point), *compar_point);
			...
			vec_time_std += vec_time_single.y;
		...
		printf("(vec) array_size: %d, sort_time (ms): %g\n", j, vec_time/times);
		...
		printf("(vec) array_size: %d, sort_time (ms): %g\n", j, vec_time_std/times);
		printf("\n");
		int_time = double_time = float_time = long_time = point_time = vec_time = 0;
		int_time_std = double_time_std = long_time_std = float_time_std = point_time_std = vec_time_std = 0;
	}
}
...
Point time_qsort_array(void* arri_1, size_t num, size_t size, int (*compar) (const void*, const void*)) 
{
	...
	Point p = { .x = difference , .y = difference_std };
